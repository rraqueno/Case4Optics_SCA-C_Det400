...
;
;
pro fly_sca2_detector400_output_matrix_rotated_weighted, image_filename, ghost_map_filename, $
...
ghost_weights = radiance_array[*,*,0]
...
for band = 0,n_positions-1 do begin  
    ...
    line_pos = sorted_indices[ 1, band ]
      ...
      temp[gt_zeroes] = goes_subsection[gt_zeroes] * ghost_weights[gt_zeroes]
...
wset,0
  ...
  envi_write_envi_file, radiance_array,out_name=output_file_basename+'-unsampled'+'-weighted.img',bnames=["Ghost Weights",band_names], map_info=map_info

  envi_write_envi_file, ghost_radiance_array,out_name=output_file_basename+'-'+'sampled'+'-weighted.img',bnames=["Ghost Weights",band_names], map_info=map_info
...
for i = 0, n_positions do begin
  ...
  envi_write_envi_file, flicker_array,out_name=output_file_basename+'-'+'flicker'+'-weighted.img'
